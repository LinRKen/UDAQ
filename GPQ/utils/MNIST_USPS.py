...
def get_pic_label(dir:str):
    file_dir = dir+'npy_files/'
    import os
    file_list = os.listdir(file_dir)
    aim_file = 'pic.npy'
    if aim_file not in file_list:
        list_dir = dir+'all_list.txt'
        data_pic,data_label = produce_pic(dir,list_dir)
        np.save(file_dir+'pic.npy',data_pic)
        np.save(file_dir+'label.npy',data_label)
        return data_pic,data_label
    data_pic = np.load(file_dir+'pic.npy')
    data_label = np.load(file_dir+'label.npy')
    return data_pic,data_label

def deal_MNIST_USPS():
    print('MNIST_USPS data load start')
    S_pic,S_label = get_pic_label('/mnt/hdd1/zhangzhibin/dataset/MNIST/')
    T_pic,T_label = get_pic_label('/mnt/hdd1/zhangzhibin/dataset/USPS/')

    S_pic = color_preprocessing(S_pic)
    T_pic = color_preprocessing(T_pic)
    print('MNIST_USPS data load OK')
