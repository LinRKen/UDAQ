...
def PQ_retrieval(sess, x, training_flag, feature, Z, n_book, db_x, test_x, label_Similarity, visual_flag, TOP_K=54000):
    ...
    batch_size = 100
    iteration = int(train_data_num / batch_size)

    batch_size_test = 100 
    test_iteration = int(test_data_num / batch_size_test)
    ...
    for step in range(iteration + 1):
        ...
        # print('train_features_batch shape = ',batch_x.shape)
        if step == 0:
    ...
    # print('quantizedDist shape = ',quantizedDist.shape)

    Rank = np.argsort(quantizedDist, axis=0)
    # print('Rank shape = ',Rank.shape)
    # for i in range(Rank[0].shape[0]):
    #     print(Rank[0][i],end=' ')
    # exit()
