...
class GPQ():
    def __init__(self, training,args):
        ...
        self.intn_word=args.intn_word
        self.n_book=args.n_book
        self.Z = tf.get_variable('Z', [self.intn_word, len_code * self.n_book], dtype=tf.float32,
                                   initializer=initializer, trainable=True)
        self.Prototypes = tf.get_variable('C', [n_CLASSES, len_code * self.n_book], dtype=tf.float32,
                                         initializer=initializer, trainable=True)
    ...
    def F(self, input_x):
        with tf.variable_scope('Fixed_VGG', reuse=tf.AUTO_REUSE):
            ...
            x = Linear(x, len_code * self.n_book, layer_name='feature_vector')
    ...
    def C(self, features, Prototypes):
        with tf.variable_scope('C', reuse=tf.AUTO_REUSE):
            x = tf.split(features, self.n_book, 1)
            y = tf.split(Prototypes, self.n_book, 0)
