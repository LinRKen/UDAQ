...
from my_dataset import get_CIFAR10
...
from base_models import model_fc, model_vgg,model_vgg_single_head
...
if __name__ == '__main__':
    ...
    if args.dataset == 'DomainNet':
        ...
        n_class = 10
        backbone = model_vgg_single_head(dim,n_class).cuda()
    ...
    for epoch in range(n_train):
        ...
        if epoch % val_epoch == 0:
            ...
            if args.dataset == 'DomainNet':
                ...
                test_mAP(S_query_set,S_gallery_set,backbone,Q=quantizer,top_k=-1,dim=dim, logger=logger)

            logger.info(f'test cross-domain quantized = ')
            mAP = test_mAP(T_query_set,S_gallery_set,backbone,Q=quantizer,top_k=-1,dim=dim, logger=logger)
