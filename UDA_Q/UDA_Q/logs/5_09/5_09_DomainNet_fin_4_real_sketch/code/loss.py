...
def Balance_loss(feat,quantizer):
    ...
    # loss = ( prob_code*( prob_code.log() ) ).sum(dim=1).mean()
    # print(loss.size())
    # loss = loss.sum(dim=1)
    # print(loss.size())
    # loss = loss.mean()
...
def trust_loss(args,T_f,quantizer,prob_CB_cls):
    ...
    if threshold > 0 :
        clip_predict_prob = predict_prob[ predict_prob >= threshold ]
        if predict_prob.max() < threshold:
            loss_adaptive= -predict_prob.mean()*1e-7
        else:
            loss_adaptive = -clip_predict_prob.mean()
    else:
        soft_weight = torch.zeros_like( predict_prob)
        soft_weight.requires_grad_(False)
        trust_gamma = args.trust_gamma
        soft_weight.data = torch.exp( trust_gamma*(predict_prob.detach()-1) )
        # soft_weight.data = 1/(1+torch.exp( -trust_gamma*(predict_prob.detach()-0.5) ) )
        loss_adaptive = -( soft_weight*predict_prob).mean()
        # loss_adaptive = -( soft_weight*predict_prob).sum()/( soft_weight.sum() +1e-9)
    # soft_weight = (predict_prob-MIN)/(MAX-MIN)
    # soft_weight.data = predict_prob.detach()**3
    # soft_weight = predict_prob.detach()

    # soft_weight = predict_prob

    # print('mean_w = ',soft_weight.mean())
    # print('min_w = ',soft_weight.min())
    # print('max_w = ',soft_weight.max())
    # print('mean = ',mean)
    # print('std = ',std)
    # print('MIN = ',MIN)
    # print('MAX = ',MAX)
