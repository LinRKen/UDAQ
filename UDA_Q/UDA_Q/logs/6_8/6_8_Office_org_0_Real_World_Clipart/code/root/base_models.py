...
class model_simple_fc_MNIST(nn.Module):
    def __init__(self,dim,n_class):
        super(model_simple_fc_MNIST, self).__init__()
        self.n_class = n_class

        self.bn_fc = nn.BatchNorm1d(25088)

        self.fc = nn.Linear(25088, dim)
        nn.init.xavier_uniform_(self.fc.weight)
        torch.nn.init.constant_(self.fc.bias.data, 0.1)
    ...
    def forward(self, x):
        feat = self.tanh( self.fc( self.bn_fc(x) ) )

        predict = torch.randn(x.size(0),self.n_class).cuda()
