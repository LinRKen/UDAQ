...
def train_source( epoch , core ,train_loader1, model,list_optimizer,quantizer=None ):
    ...
    center = torch.zeros(345, 512).cuda()
    ...
    for n_iter, (S_img,S_label,_,_,S_idx) in enumerate(train_loader1):
        ...
        center[S_label] = center[S_label]*0.9+S_feat*0.1
...
def train_S_T_pair( epoch , core ,train_loader,T_train_set, model, list_optimizer, quantizer=None ):
    ...
    for n_iter, (S_img,T_img,S_label,T_pseudo_label,S_idx,T_idx) in enumerate(train_loader):
        ...
        S_loss_gather = torch.norm( S_feat-center[S_label] , p=2,dim=1).mean()

        # T_loss_gather = torch.norm( T_feat-core[T_pseudo_label] , p=2,dim=1).mean()
        T_loss_gather = torch.norm( T_feat-center[T_pseudo_label] , p=2,dim=1).mean()
        # T_loss_gather = torch.zeros(1).cuda()

        center[S_label] = center[S_label]*0.9+S_feat*0.1
        center[T_pseudo_label] = center[T_pseudo_label]*0.9+T_feat*0.1
        ...
        if quantizer == None:
            ...
            mean_mse.add( torch.norm(S_feat-Q_S_feat,p=2,dim=1).mean() , S_img.size(0) )
