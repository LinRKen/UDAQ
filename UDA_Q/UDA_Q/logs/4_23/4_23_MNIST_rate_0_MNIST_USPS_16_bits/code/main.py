...
from my_dataset import get_CIFAR10, get_ImageNet, get_img_DomainNet
...
from base_models import model_simple_fc,model_fc_cls,model_refix_fc
...
# model_fc, model_vgg,model_AlexNet_single_head,model_S_T_fc,model_vgg_single_head
    if type(backbone) in [model_fc,model_S_T_fc,model_fc_cls,model_refix_fc]:
        S_optimizer = optim.SGD(backbone.parameters(), lr=args.lr, momentum=0.9, nesterov=True)
        # T_optimizer = optim.SGD(backbone.parameters(), lr=args.lr, momentum=0.9, nesterov=True)
        T_optimizer = S_optimizer
        # S_optimizer = optim.Adam(backbone.parameters(), lr=args.lr)
        # T_optimizer = optim.Adam(backbone.parameters(), lr=args.lr)
        # ,betas=(0.5,0.999))
    elif type(backbone) in [ model_AlexNet_single_head,model_vgg_single_head ] :
        optimizer = optim.Adam(backbone.parameters(), lr=args.lr)
        # decayed_lr = tf.train.exponential_decay(0.002, global_step, 100, 0.95, staircase=True)
    ...
    val_epoch = 20
    if args.dataset in ['DomainNet','MNIST_USPS']:
        val_epoch = 10
    ...
    if False:
        st_epoch = 30
        load_model_path = '/home/zhangzhibin/data/UDA/CDTrans-master/UDA_Q/logs/4_22/4_22_MNIST_only_S_pic_224_0_MNIST_USPS_16_bits/model/4_22_MNIST_only_S_pic_224_0_40.pth'
    ...
    for epoch in range(st_epoch,n_train+1):
        ...
        if epoch>= epoch_warmup:
            ...
            S_rate = 1
        S_loss = train_source( S_train_loader, backbone, [S_optimizer,Q_optimizer],quantizer,logger)
        # mean_loss = 0
        ...
        if epoch >= epoch_warmup:
            ...
            # quantizer,logger )
            T_loss = train_T_sample( T_train_loader,S_train_loader, backbone, [T_optimizer], 
            quantizer,logger )
        ...
        if epoch % val_epoch == 0:
            ...
            if args.dataset in ['DomainNet','OfficeHome','MNIST_USPS']:
                ...
                test_mAP(S_query_set, S_gallery_set,True,True,backbone,top_k=top_k, dim=dim, logger=logger)
            
            logger.info(f'test cross-domain = ')
            test_mAP(T_query_set, S_gallery_set,False,True, backbone,top_k=top_k, dim=dim, logger=logger)


            if args.dataset  in ['DomainNet','OfficeHome','MNIST_USPS']:
                logger.info(f'test single-domain quantized = ')
                test_mAP(S_query_set,S_gallery_set,True,True,backbone,Q=quantizer,top_k=top_k,dim=dim, logger=logger)

            
            logger.info(f'test cross-domain quantized = ')
            mAP = test_mAP(T_query_set,S_gallery_set,False,True,backbone,Q=quantizer,top_k=top_k,dim=dim, logger=logger)
