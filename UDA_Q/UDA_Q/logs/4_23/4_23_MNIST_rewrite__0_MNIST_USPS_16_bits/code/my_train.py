...
def train_source( args,S_train_loader, model,list_optimizer,prob_CB_cls,quantizer=None,logger=None,S_rate=1 ):
    ...
    with torch.no_grad():
        ...
        if prob_CB_cls is None:
            prob_CB_cls = quantizer.get_prob_CB_cls(S_f,S_label,model.n_class)
        else:
            prob_CB_cls = prob_CB_cls*args.cls_gamma+quantizer.get_prob_CB_cls(S_f,S_label,model.n_class)*(1-args.cls_gamma)
    ...
    for n_iter, data in enumerate(S_train_loader):
        ...
        # random_noisy = torch.normal(mean=torch.zeros_like(S_base_f),std=torch.ones_like(S_base_f)*1e-3)
        # S_base_f += random_noisy
        # print(random_noisy)
        # exit()
        # for i in range(4096):
        #     if S_base_f[0][i]!=0:
        #         print(f'{S_base_f[0][i].item():.3f}',end=' ')
        # exit()
...
def train_T_sample(T_train_loader,S_train_loader, backbone, list_optimizer , quantizer,logger ):
    ...
    with torch.no_grad():
        ...
        prob_CB_cls = quantizer.get_prob_CB_cls(S_f,S_label,backbone.n_class)
    ...
    for n_iter, (T_base_f,T_pseudo_label) in enumerate(T_train_loader):
        ...
        # random_noisy = torch.normal(mean=torch.zeros_like(T_base_f),std=torch.ones_like(T_base_f)*1e-3)
        # T_base_f += random_noisy
