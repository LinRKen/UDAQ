...
from base_models import model_simple_fc,model_fc_cls
...
# model_fc, model_vgg,model_AlexNet_single_head,model_S_T_fc,model_vgg_single_head
    if type(backbone) in [model_fc,model_S_T_fc,model_fc_cls]:
        optimizer = optim.SGD(backbone.parameters(), lr=args.lr, momentum=0.9, nesterov=True)
        # optimizer = optim.Adam(backbone.parameters(), lr=args.lr,betas=(0.5,0.999))
    ...
    for epoch in range(st_epoch,n_train+1):
        ...
        logger.info('lr = '+str(optimizer.param_groups[-1]['lr']))
        ...
        if epoch>= epoch_warmup:
            ...
            S_rate = 1
        mean_loss = train_source( S_train_loader, backbone, [optimizer,Q_optimizer],quantizer,logger,S_rate)
        # mean_loss = 0
        ...
        if epoch >= epoch_warmup:
            ...
            # quantizer,logger )
            mean_loss += train_T_sample( T_train_loader,S_train_loader, backbone, [optimizer], 
            quantizer,logger )
