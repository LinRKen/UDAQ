...
def split_list(list_dir,num_tr_pics):
    ...
    query_list = all_list[:num_tr_pics]
    train_list = all_list[num_tr_pics:]
...
def get_MNIST_USPS():    
    list_dataset=[]
    dir = '/home/zhangzhibin/data/UDA/CDTrans-master/fixed_f/vgg/MNIST_USPS/MNIST_32/'
    for name in ['all','query','all',]:
        f_dir = dir+name+'_feature.npy'
        label_dir = dir+name+'_label.npy'
        Q_features = np.load(f_dir)
        Q_labels = np.load(label_dir)
        dataset = fixed_dataset(Q_features, Q_labels)
        list_dataset.append( dataset )
    
    dir = '/home/zhangzhibin/data/UDA/CDTrans-master/fixed_f/vgg/MNIST_USPS/USPS_32/'
