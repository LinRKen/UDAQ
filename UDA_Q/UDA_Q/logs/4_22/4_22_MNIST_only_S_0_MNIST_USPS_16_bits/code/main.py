...
from base_models import model_squeeze_fc
...
if __name__ == '__main__':
    ...
    if args.dataset == 'DomainNet':
        ...
        # backbone = model_squeeze_fc(dim,args.n_class).cuda()
        backbone = model_fc(dim,args.n_class).cuda()
...
# model_fc, model_vgg,model_AlexNet_single_head,model_S_T_fc,model_vgg_single_head
    if type(backbone) in [model_fc,model_S_T_fc,model_squeeze_fc]:
        optimizer = optim.SGD(backbone.parameters(), lr=args.lr, momentum=0.9, nesterov=True,weight_decay=1e-5)
