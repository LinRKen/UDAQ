...
def generate_feature(dataloader, backbone):
    backbone.eval()
    label_list = []
    ...
    cnt = 0
    with torch.no_grad():
        for n_iter, (img,batch_label) in enumerate(dataloader):
            img = img.cuda()
            batch_label = batch_label.cuda()

            batch_f,_ = backbone(img)
            if n_iter == 0:
                f = batch_f
                label = batch_label
            else:
                f = torch.cat( (f,batch_f) , dim=0)
...
def prob_pseudo_label(S_train_loader, T_train_loader, backbone, quantizer,threshold,logger):
    backbone.eval()
    T_train_set = T_train_loader.dataset
    ...
    with torch.no_grad():
        ...
        logger.info(f' mean prob = {prob_pseudo_label.mean().item():.3f}')
        for part_rate in [0.1,0.3,0.5,0.7]:
        ...
        if threshold > 0:
            ...
            idx = sorted_idx[ : int(T_f.size(0)*(-threshold) )]
        ...
        #     data_list.append( ( part_base_f[i],part_T_label[i].item()) )

        n_data = part_T_label.size(0)
